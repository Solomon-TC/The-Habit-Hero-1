-- Final fix for friend display names

-- 1. Ensure the users table has the display_name column
...
-- 4. Create a function to get a user's display name
...
BEGIN
    ...
    IF display_name IS NULL OR display_name = '' THEN
        ...
        ON CONFLICT (id) DO UPDATE SET 
            ...
            name = CASE WHEN public.users.name IS NULL OR public.users.name = '' OR public.users.name = public.users.id::text THEN EXCLUDED.name ELSE public.users.name END,
...
-- 5. Create a function to ensure all friend users exist with proper display names
...
-- 6. Create a function to directly get friend data with proper names
...
-- 7. Run the function to fix existing data
